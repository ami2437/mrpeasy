...
print("="*80)
print("FINAL CATEGORIZATION (3 CATEGORIES)")
print("="*80)
print()

# NOT INVOICED
print("📋 NOT INVOICED: Shipped but NO invoice created (invoiced = 0)")
print("-" * 80)
print(f"Total orders: {len(not_invoiced_orders)}\n")
for idx, order in enumerate(not_invoiced_orders[:3], 1):  # Show first 3
    print(f"{idx}. {order['code']} - {order['customer']}")
    for item in order['items'][:2]:
        print(f"   • {item['item_code']}: shipped={item['shipped_qty']}, invoiced=0, missing={item['discrepancy']}")
    if len(order['items']) > 2:
        print(f"   ... and {len(order['items']) - 2} more items")
    print()
if len(not_invoiced_orders) > 3:
    print(f"... and {len(not_invoiced_orders) - 3} more orders\n")

print()
print("="*80)
print("⚠️  UNDER-INVOICED: Partial invoices (invoiced > 0 but < shipped)")
print("-" * 80)
...
print()
print("="*80)
print("🔴 OVER-INVOICED: Invoice exceeds shipment (invoiced > shipped)")
print("-" * 80)
...
print(f"Total: {len(not_invoiced_orders) + len(under_invoiced_orders) + len(over_invoiced_orders)}")
print("="*80)
